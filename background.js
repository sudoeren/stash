...
chrome.runtime.onInstalled.addListener(() => {
    chrome.contextMenus.create({ id: 'save-current-tab', title: 'Bu sekmeyi Stash\'e kaydet', contexts: ['page'] });
    chrome.contextMenus.create({ id: 'save-all-tabs', title: 'Tüm sekmeleri Stash\'e kaydet', contexts: ['page'] });
    chrome.contextMenus.create({ id: 'open-dashboard', title: 'Stash\'i aç', contexts: ['page'] });
});
...
async function saveCurrentTab(tab) {
    ...
    const group = { id: generateId(), createdAt: new Date().toISOString(), favorite: false, tabs: [{ id: generateId(), title: tab.title || 'Başlıksız', url: tab.url, favicon: tab.favIconUrl || null }] };
...
async function saveAllTabsBackground() {
    ...
    const group = { id: generateId(), createdAt: new Date().toISOString(), favorite: false, tabs: filteredTabs.map(tab => ({ id: generateId(), title: tab.title || 'Başlıksız', url: tab.url, favicon: tab.favIconUrl || null })) };
    tabGroups.unshift(group);
    await chrome.storage.local.set({ tabGroups });
